...
def fifth_window(v_from, v_to, set_a, set_c):
    ...
    fifth_window.configure()
    fifth_window.title('Вікно 5')
    fifth_window.geometry('500x580')

    menu_label = Label(fifth_window, text="Елементи множин A, B, C", font="30")
    menu_label.grid(column=1, row=1, pady=20, columnspan=3)

    set_a_label = Label(fifth_window, text="X", font="20")
    set_a_label.grid(column=1, row=2, sticky=W, pady=5, padx=5)

    set_b_label = Label(fifth_window, text="Y", font="20")
    set_b_label.grid(column=1, row=3, sticky=W, pady=5, padx=5)

    set_a_input = Entry(fifth_window, font="20", width="40")
    ...
    set_b_input = Entry(fifth_window, font="20", width="40")
